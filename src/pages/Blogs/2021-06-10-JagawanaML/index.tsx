...
const JagawanaML = () => {
  ...
  return (
    <BlogLayout
      ...
      date='10 June 2021'
    >
      <Image alt='soundwave' src='https://imgur.com/kPlY4p6.png' />
      <Quote>
        Jagawana is a Wide Sensor Network System deployed in the forests to
        prevent Ilegal Logging. By using sensors to pick up voices in the
        forests, we could monitor what happened in the forest in real-time. We
        deployed a Machine Learning Model to process the sounds taken by the
        sensor, then the model will identify the sounds into various categories,
        such as chainsaws, trucks, gunshot, and burning sounds. We will be using
        Android App to monitor and notify the user if suspicious events were
        happening in the forest, the user could also be able to hear the sound
        itself to ensure the results are correct.
      </Quote>
      ...
      <Body>
        Google's Audioset data consists of CSV data with youtube links,
      ...
      <Body>
        There are many image recognition examples in online courses and
        tutorials. The way machine learning classifies an image is by analyzing
        the pixel value as a series. Based on this analogy, we should be able to
        analyze sounds based on the amplitude as a time series, but this is
        actually not the case.
      </Body>
      <ImageCaption
        src='https://i.imgur.com/IwUTECt.jpg'
        alt='waveform mel'
        caption='Waveform and Mel Spectogram'
      />
      <Body>
        A waveform is an amplitude measurement put in a time series. There are
        many disadvantages to analyze the waveform, the data is big and messy,
        and the machine learning model doesn't perform well. The way around it
        is by analyzing the audio data and turn it into features, by analyzing
        it on the frequency domain (rather than amplitude) using math. In our
        ...
        above. Using Mel Spectrogram which has less size than the actual
        waveform, we can train our model faster and get better performance too.
      </Body>
      <Body>
        There are other features that we can extract from a waveform, like
        normal Spectrogram, MFCC, and many else. Here is a reference you can use
        to learn more about{' '}
        <a
      ...
      <Body>
        ...
        million images belonging to 1000 classes. VGG16 is consisted of 16
        layers, with over 140M parameters and a size of 533MB. This makes
        training and deploying the model a tiresome task.
      </Body>
      <Body>
      ...
      <ol className='pl-8 list-decimal'>
        ...
        <li>
          Google Audioset; all 3 classes exist here, the dataset itself is a
          list of associated YouTube ID, start time, end time, and class labels
          in a CSV file. To use the dataset as audio data, we need to download
          the recordings from youtube using a script that I have modified{' '}
          <a
            href='https://github.com/nicorenaldo/audioset-processing'
            className='font-bold underline'
          >
      ...
      <Body>
        ...
        since we have various length of data, we decided to use only 2 seconds
        of recordings at a time. To get such data, we have to do some
        preprocessing.
      </Body>
      <ol className='pl-8 list-decimal'>
        <li>
      ...
      <Body>
        In our experiment, we are using 320 clips for each classes. The data
        which is still in the waveform then will be processed into Mel
        Spectrogram and shuffled into the train and validation dataset.
      </Body>
      <Body>
      ...
      <ImageCaption
        src='https://i.imgur.com/KlDjS2r.jpg'
        alt='confusion matrix'
        caption='Accuracy & Loss History and Confusion Matrix'
      />
      <Body>
        From the confusion matrix, we could see the model perform quite nicely
        for all 3 classes.
      </Body>
      ...
      <Body>
        ...
        actually pass the whole audio data to the model, that is because the
        model could use any input size, but this method will perform worse than
        other methods. Another way to do it is by predicting a window of n
        seconds audio, and shifting it 1 second at a time, this method really
        works well but comes at a price, the computational needed will be huge.
      </Body>
      <Body>
        ...
        The target sounds will have a set of intervals, and the model will
        predict each highlighted clip separately. You can see the illustration
        of the method in the image below. After reducing the noise, the output
        results into 5 highlighted clips.
      </Body>
      <ImageCaption
        src='https://i.imgur.com/KAGbKKJ.png'
        alt='highlighting sound'
      ...
      <Body>
        ...
        using a different audio feature like MFCC. Feel free to ask questions
        and using my Kaggle notebook as a reference, I hope my work on this
        project could help someone else in researching this topic.
      </Body>
    </BlogLayout>
  );
};
